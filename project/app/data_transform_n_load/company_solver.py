...
# constants
from app.constants import SYMBOL_KEY, OVERVIEW_FUNCTION
...
class CompanySolver:
    ...
    def get_company_data(self, db: Session) -> CompanyResponseModel:
        company_summary = {}
        company = (
            db.query(Company).filter(Company.symbol == self.company_symbol).first()
        )
        if not company:
            ...
            company = load_company.set_commpany_data(
                all_company_data=all_company_data, db=db
            )
            load_company.set_all_company_earning_anual(
                all_company_data=all_company_data, db=db
            )
            load_company.set_all_company_earning_quarterlies(
                all_company_data=all_company_data, db=db
            )
        response_model = {
